...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'tree-view:remove-project-folder': (event) => {
        console.log("CAPTURADO!!!!!!!!!!");
        //obj = ApioBuildProvider.instances.pop();
        //obj.enable = false;
        //console.log(ApioBuildProvider.instances.length);
      }
...
class ApioBuildProvider {
  ...
  constructor(cwd) {
    ...
    console.log("Nuevo Builder!!!");
    ApioBuildProvider.p += 1;
    this.id = ApioBuildProvider.p;
    console.log(this.id);
  }
  ...
  isEligible() {
    //-- Only the first open folder can be built. Not the rest
    if (this.id == 1) {
      return true;
    }
    else {
...
ApioBuildProvider.p = 0;
ApioBuildProvider.instances = [];
